...
export const ease = Easing.bezier(0.22, 0.75, 0.12, 1);
...
export const Label: React.FC<{
  ...
  y?: number;
  light?: boolean;
}> = ({ children, x = 130, y = 110, light = false }) => (
  <div
    style={{
      position: "absolute",
      left: x,
...
export const PaperCutout: React.FC<{
  ...
  delay?: number;
  fit?: "cover" | "contain";
}> = ({ src, x, y, w, h, angle = 0, delay = 0, fit = "cover" }) => {
  const f = useCurrentFrame();
  const p = progress(f, delay, delay + 22);
  return (
    <div
      style={{
        position: "absolute",
        ...
        width: w,
        height: h,
        rotate: `${angle + (1 - p) * 3}deg`,
        translate: `0px ${(1 - p) * 90}px`,
        opacity: p,
        filter: "drop-shadow(0px 16px 14px #00000024)",
      }}
    >
      <div
        style={{
          ...
          clipPath:
            "polygon(0 1%,15% 0,29% 1%,41% 0,59% 1%,75% 0,100% 1%,99% 27%,100% 48%,99% 72%,100% 99%,79% 100%,62% 99%,44% 100%,29% 99%,0 100%,1% 73%,0 54%,1% 31%)",
        }}
      />
      <Img
        src={staticFile(src)}
...
export const DocumentaryPhoto: React.FC<{
  src: string;
  focus?: string;
  children?: React.ReactNode;
}> = ({ src, focus = "50% 50%", children }) => {
  const f = useCurrentFrame();
  return (
    <AbsoluteFill style={{ overflow: "hidden", background: palette.ink }}>
      <Img
        ...
        style={{
          ...
          scale: interpolate(f, [0, 450], [1.05, 1.12], clamp),
          translate: `${interpolate(f, [0, 450], [-15, 15], clamp)}px 0px`,
        }}
      />
      <AbsoluteFill
        style={{
          background:
            "linear-gradient(0deg,rgba(15,22,26,.72),transparent 48%)",
        }}
      />
      {children}
      <TextureOverlay />
...
export const Camera: React.FC<{
  ...
  duration?: number;
  to?: number;
}> = ({ children, duration = 300, to = 1.08 }) => {
  const f = useCurrentFrame();
  return (
    <AbsoluteFill
      style={{ scale: interpolate(f, [0, duration], [1, to], clamp) }}
    >
      {children}
    </AbsoluteFill>
  );
