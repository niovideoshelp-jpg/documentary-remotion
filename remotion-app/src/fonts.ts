...
if (typeof document !== 'undefined') {
  ...
  Promise.all([
    ...
    new FontFace('Oswald',`url("${staticFile('fonts/Oswald.ttf')}")`,{weight:'200 700'}).load(),
  ]).then(fonts=>{fonts.forEach(font=>document.fonts.add(font));continueRender(handle);}).catch(cancelRender);
}
